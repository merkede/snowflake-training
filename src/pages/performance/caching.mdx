...
moduleId: "caching"
domain: "Performance"
description: "Master Snowflake's three-layer caching system — result cache, local disk cache, and metadata cache — to maximise performance and reduce costs for the SnowPro Core exam"
---
...
<KeyTerms client:load title="Key Terms — Caching Mechanisms" terms={[
  {
    term: "Result Cache",
    abbr: "QRC",
  ...
  },
  {
    term: "Metadata Cache",
    abbr: "MDC",
    definition: "A persistent, Snowflake-managed store in the Cloud Services layer that holds table and column statistics — row counts, min/max values, null counts, and distinct value estimates — used for query planning and pruning."
  },
  {
    term: "Cache Hit",
  ...
  },
  {
    ...
    definition: "A SQL function that can return different results on successive calls with identical inputs — for example, CURRENT_TIMESTAMP(), RANDOM(), and UUID_STRING(). Queries containing these functions cannot be served from the result cache."
  },
  {
    term: "Micro-Partition",
    definition: "Snowflake's internal storage unit. Each micro-partition is a contiguous, columnar, compressed file of approximately 50–500 MB of uncompressed data. Caching operates at this granularity."
  },
  {
...
<CalloutBox type="exam" title="Exam Priority — Know All Three Caches">
...
<StepByStep client:load title="Optimising Local Disk Cache Warming for Repeated Workloads" steps={[
  ...
  },
  {
    ...
    tip: "Scaling up increases both compute power and SSD cache capacity — but only scale up if cache misses are the identified bottleneck, not simply slow query execution."
  }
]} />
...
<CalloutBox type="info" title="Queries Answered Entirely from Metadata — No Warehouse Required">
...
- `SELECT COUNT(*) FROM table_name` with no WHERE clause — the row count is in metadata
- `SELECT MIN(clustering_key_col), MAX(clustering_key_col) FROM table_name` — min/max from metadata
- `SHOW TABLES`, `SHOW SCHEMAS`, `SHOW COLUMNS` — entirely metadata-served
- `DESCRIBE TABLE table_name` — metadata only
...
<CompareTable client:load title="Result Cache vs Local Disk Cache" leftLabel="Result Cache" rightLabel="Local Disk Cache" rows={[
  ...
  },
  {
    ...
    left: "Yes — any warehouse can hit a result cached by any other warehouse",
    right: "No — each warehouse has its own isolated SSD cache",
    winner: "left"
  },
  {
    feature: "Credit cost on cache hit",
    left: "Zero credits — no warehouse activation",
  ...
  },
  {
    ...
    left: "Blocks cache hit — query must re-execute",
    right: "No impact — caches raw data regardless of function use",
    winner: "right"
  },
  {
    feature: "Role context required?",
    left: "Yes — same role as original cached query",
    right: "No — role has no bearing on SSD data cache",
    winner: "right"
  },
  {
    feature: "Exact SQL match required?",
    left: "Yes — byte-for-byte identical SQL text",
    right: "No — caches at data block level, not query level",
    winner: "right"
  },
  {
    feature: "User configuration required?",
    left: "None — always active, cannot be disabled",
    right: "None — automatic; auto-suspend tuning affects warming",
    winner: "none"
  }
]} />
...
<CalloutBox type="warning" title="Cache Invalidation — Common Exam Trap">
...
<CodeBlock client:load language="sql" title="Testing Cache Behaviour — Practical Examples" code={`-- ============================================================
-- RESULT CACHE TESTING
-- ============================================================

-- Run this query first — it executes against the warehouse
...
-- Run the EXACT same query again — should hit result cache
...
-- Now the same query will NOT hit result cache — cache invalidated
SELECT region, SUM(sales_amount) AS total_sales
FROM fact_sales
WHERE sale_date BETWEEN '2024-01-01' AND '2024-03-31'
GROUP BY region
...
-- ============================================================
-- METADATA CACHE EXAMPLES — NO WAREHOUSE NEEDED
-- ============================================================

-- COUNT(*) on large table — served from metadata cache
-- No warehouse credit consumption
SELECT COUNT(*) FROM fact_sales;

-- MIN/MAX on well-clustered column — from metadata
...
<CalloutBox type="tip" title="Practical Tip — Standardise SQL Formatting for Result Cache Hits">
...
<CheatSheet client:load title="Snowflake Caching — COF-C02 Quick Reference" sections={[
  {
    title: "Result Cache",
    icon: "⚡",
    items: [
...
<Flashcard client:load category="Caching" question="Do two different virtual warehouses share the local disk cache?" answer="No. The local disk cache is strictly per-warehouse. Each virtual warehouse has its own independent SSD layer. If Warehouse A caches micro-partitions for Table X, Warehouse B cannot access those cached partitions — it must fetch and cache them independently from cloud storage." />
