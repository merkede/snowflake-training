...
description: "Master the Snowflake Search Optimisation Service (SOS) — how it works, when to use it, costs, and how it differs from clustering keys for the SnowPro Core exam"
...
The **Search Optimisation Service (SOS)** is a Snowflake Enterprise-tier feature designed to dramatically accelerate **point lookup queries** on large tables. Unlike clustering keys — which help range-based scans — SOS is purpose-built for queries that search for specific values in large datasets, such as finding a customer by ID or filtering on a phone number.
...
<KeyTerms
  ...
  terms={[
    ...
    },
    {
      ...
      definition: "A query that searches for one or more specific values — e.g. WHERE user_id = 99 or WHERE email IN (...) — rather than scanning a range of rows."
    },
    {
...
Standard Snowflake query performance relies on **micro-partition pruning** — Snowflake reads the min/max metadata for each micro-partition and skips those that cannot contain the desired values. This works exceptionally well for **range queries** and **sorted data**, but breaks down for random point lookups.

Consider a table of 500 million customer records. If you query `WHERE customer_id = 'C-00019284'`, Snowflake must scan potentially thousands of micro-partitions because customer IDs are randomly distributed — no pruning is possible without a sort order on that column.

The Search Optimisation Service solves this by building a **persistent search access path** — effectively a specialised index structure — that allows Snowflake to identify precisely which micro-partitions contain the target value, skipping the rest entirely.
...
<Diagram
  ...
  imagePath="/images/diagrams/search-optimisation-illustration.png"
/>

The process works as follows:

1. **Build Phase** — When SOS is enabled on a table, Snowflake's serverless background compute builds the initial search access path. This is a one-time build cost charged in serverless credits.
2. **Maintenance Phase** — As DML operations (INSERT, UPDATE, DELETE, MERGE) change the table, Snowflake continuously and automatically updates the access path. Each update consumes additional serverless credits.
3. **Query Phase** — When a qualifying query arrives (e.g. a `WHERE id = ?` predicate on an SOS-configured column), the query optimiser consults the access path to determine which micro-partitions are relevant. Only those partitions are read by the virtual warehouse.
...
<Diagram
  ...
  imagePath="/images/diagrams/search-sos-query-profile.png"
/>
...
- **Range predicates** alone: `WHERE created_at > '2024-01-01'` — clustering keys are better here
- **Full table scans** with no selective predicate
- **JOIN performance** — SOS does not accelerate join operations
...
The exam may present a scenario with a date range filter — e.g. `WHERE order_date BETWEEN '2024-01-01' AND '2024-03-31'` — and ask which feature helps most. The answer is **clustering keys**, not SOS. SOS is for point lookups, not ranges.
</CalloutBox>
...
<StepByStep
  ...
  steps={[
    ...
    },
    {
      ...
      description: "Analyse your query patterns. SOS benefits columns used in equality filters where data is not naturally clustered — e.g. UUID-based IDs, email addresses, phone numbers. Run EXPLAIN on slow queries to confirm full partition scans.",
    ...
    },
    {
      ...
      tip: "Start with equality — it covers most point lookup use cases."
    },
    {
...
These three features are frequently compared on the exam. They are not mutually exclusive — tables can have both SOS and clustering keys simultaneously.
...
<CompareTable
  ...
  rows={[
    ...
    },
    {
      ...
      left: "Yes — both can coexist on same table",
      right: "Yes — both can coexist on same table",
      winner: "none"
    },
    {
      feature: "Requires data to be sorted?",
      left: "No — works regardless of data order",
      right: "Yes — effectiveness depends on data being clustered",
      winner: "left"
    }
  ]}
/>
...
The **result cache** serves identical query results instantly (free, 24-hour TTL). It helps when the exact same query text is repeated. SOS helps when query **structure** is the same but **values differ** — e.g. looking up a different customer ID each time. These two features solve different problems and complement each other.
</CalloutBox>
...
1. **Build cost** — A one-time serverless credit charge when SOS is first enabled and the initial access path is constructed.
2. **Maintenance cost** — Ongoing serverless credits consumed each time the access path is updated due to DML changes (INSERTs, UPDATEs, DELETEs, MERGEs).

Additionally, the search access path consumes **storage**, billed at standard Snowflake storage rates.

Query execution itself still uses your **virtual warehouse** — SOS reduces the data scanned but does not eliminate warehouse usage entirely.
</CalloutBox>
...
<CheatSheet
  client:load
  title="Search Optimisation Service — COF-C02 Cheat Sheet"
  sections={[
    {
...
<Flashcard
  ...
  answer="1. Build cost — one-time serverless credits to construct the initial search access path. 2. Maintenance cost — ongoing serverless credits consumed each time DML changes (INSERTs, UPDATEs, DELETEs) update the access path. Storage for the access path is also billed separately."
/>
...
<Flashcard
  ...
  answer="ALTER TABLE t DROP SEARCH OPTIMIZATION ON EQUALITY(column_name); — This removes the access path for the specified predicate type and column while leaving SOS active on other columns."
/>
...
<Quiz
  ...
  correct="B"
  explanation="SOS maintenance is driven by DML activity — every INSERT, UPDATE, DELETE, and MERGE triggers background updates to the search access path, each consuming serverless credits. On a table with millions of daily DML operations, this maintenance cost can be substantial. SOS does not block DML operations (A is false). Warehouses do not need to be suspended (C is false). SOS works fine on tables that receive DML (D is false) — that is precisely when maintenance costs arise."
/>
