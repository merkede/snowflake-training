...
description: "Master Snowflake's RBAC model — privileges, GRANT/REVOKE, object ownership, and discretionary access control for the SnowPro Core exam"
moduleId: "rbac"
domain: "Security"
---
...
Role-Based Access Control is the primary security model in Snowflake. Every action a user performs — reading a table, running a query, creating a schema — is governed by **privileges** granted to **roles**, which are then assigned to **users**. Understanding RBAC thoroughly is essential for the COF-C02 exam.
...
A user activates a role in their session, and that role determines what the user can do. If the role does not have the required privilege on the required object, the action is denied — even if the user is an ACCOUNTADMIN in another context.

<Diagram client:load title="RBAC Hierarchy: Users, Roles, Privileges, and Objects" description="Diagram showing the RBAC chain: Users are assigned Roles. Roles hold Privileges. Privileges are granted ON Securable Objects (account, database, schema, table, warehouse). Role inheritance flows upward — a parent role inherits all privileges of its child roles. The ACCOUNTADMIN role sits at the top, inheriting from SYSADMIN and SECURITYADMIN. SYSADMIN inherits from any custom roles beneath it." altText="Snowflake RBAC hierarchy diagram showing users, roles, privileges, and securable objects"   imagePath="/images/diagrams/role-hierarchy.png"
/>
...
<CalloutBox type="exam" title="Exam Tip — Object Hierarchy">
...
<KeyTerms client:load title="Key Terms — System-Defined Roles" terms={[
...
- **REVOKE CURRENT GRANTS** — removes all existing privilege grants on the object from other roles. The new owner starts fresh.
- **COPY CURRENT GRANTS** — preserves all existing privilege grants. Other roles retain their previously granted privileges.
...
- In DAC, object **owners** can grant access to their own objects to other roles — without requiring a central admin to do so.
...
<CodeBlock client:load language="sql" title="Role Hierarchy — GRANT ROLE" code={`-- Create custom roles
...
- **CASCADE**: The revoke propagates downward — any grants that the target role made using this privilege are also revoked.
...
The exam often presents a scenario where a downstream role loses access unexpectedly — this is caused by a CASCADE revoke up the chain.
</CalloutBox>
...
<CodeBlock client:load language="sql" title="FUTURE GRANTS — Auto-grant on New Objects" code={`-- Automatically grant SELECT on any future tables created in this schema
...
**Future grants** apply to objects created **after** the grant is set up — they do not backfill existing objects.
...
<CompareTable client:load title="RBAC vs DAC in Snowflake" leftLabel="RBAC (Role-Based)" rightLabel="DAC (Discretionary)" rows={[
  ...
  {feature: "Flexibility", left: "Lower — requires admin intervention for each grant", right: "Higher — owner can delegate independently", winner: "right"},
  {feature: "Governance", left: "Stronger — all grants managed centrally", right: "Weaker — distributed, harder to audit", winner: "left"},
...
<CalloutBox type="exam" title="Least Privilege — A Core Exam Theme">
...
<CheatSheet client:load title="RBAC Quick Reference — COF-C02" sections={[
  ...
  {title: "Schema Privileges", icon: "📁", items: [
  ...
  {title: "Table Privileges", icon: "📊", items: [
  ...
  {title: "Warehouse Privileges", icon: "⚙️", items: [
  ...
  {title: "Key GRANT Commands", icon: "🔑", items: [
  ...
  {title: "SHOW GRANTS Commands", icon: "🔍", items: [
...
1. **Access requires privileges at every level** — database, schema, and object.
2. **USAGE is the gateway** — without USAGE on a database and schema, object-level privileges are useless.
3. **OWNERSHIP is exclusive** — only one role owns an object; transfer with `GRANT OWNERSHIP ... REVOKE CURRENT GRANTS`.
4. **Privilege inheritance flows upward** — parent roles inherit all privileges of child roles.
...
7. **Least privilege** is the guiding principle — always grant the minimum required.
