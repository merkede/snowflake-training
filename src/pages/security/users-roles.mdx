...
Users in Snowflake are created with the `CREATE USER` statement. Only the **login name** (which defaults to the user name) is required — all other properties are optional but important to understand for the exam.

<CodeBlock client:load
  language="sql"
  title="CREATE USER — Full Syntax with All Key Properties"
...
<CodeBlock client:load
  language="sql"
  title="ALTER USER — Modifying User Properties"
...
By default, only **SECURITYADMIN** and **ACCOUNTADMIN** can create, alter, and drop users. The **USERADMIN** system role can also create and manage users and roles — that is its specific purpose. Custom roles can be granted the `CREATE USER` privilege on the account, but this is less common.
</CalloutBox>
...
<Diagram client:load
  ...
  imagePath="/images/diagrams/system-role-hierarchy.png"
/>

### The Five System Roles in Detail

<CalloutBox client:load type="important" title="ACCOUNTADMIN — The Highest Privilege Role">
...
<CalloutBox client:load type="info" title="SYSADMIN — Database & Warehouse Management">
**SYSADMIN** is granted the ability to create and manage all **database objects** — databases, schemas, tables, views, stages, file formats, pipes, tasks, warehouses, and sequences. It is the appropriate role for day-to-day object management in most organisations. By convention, custom roles are granted to SYSADMIN so that SYSADMIN can manage objects owned by those custom roles.
</CalloutBox>

<CalloutBox client:load type="info" title="SECURITYADMIN — Access Control Management">
...
<CalloutBox client:load type="info" title="USERADMIN — User and Role Creation Only">
...
<CalloutBox client:load type="note" title="PUBLIC — The Default Role for All Users">
...
Snowflake's RBAC model supports **role hierarchies** — a role can be granted to another role, causing the parent role to **inherit all privileges** of the child role. This is how Snowflake's system roles are structured, and it is the recommended pattern for custom roles as well.

<Diagram client:load
  title="Custom Role Hierarchy — Best Practice Pattern"
  ...
  altText="Hierarchy diagram: ACCOUNTADMIN at top → SYSADMIN → analytics_role, data_engineer_role, reporting_role (all as children of SYSADMIN). Users jane, bob, and carol are granted their respective custom roles. Arrows show privilege inheritance flowing upward."
  imagePath="/images/diagrams/role-hierarchy.png"
/>

<CodeBlock client:load
  language="sql"
  title="Building a Role Hierarchy — Complete Example"
...
- `GRANT ROLE child_role TO ROLE parent_role` — creates a **role hierarchy** (parent inherits child's privileges)
- `GRANT ROLE my_role TO USER jane` — **assigns** a role to a specific user
...
<StepByStep client:load
  ...
  steps={[
    ...
    },
    {
      ...
      description: "Switch to SECURITYADMIN to create the role. Include a COMMENT to document its purpose — this is visible in SHOW ROLES and INFORMATION_SCHEMA.APPLICABLE_ROLES.",
...
CREATE ROLE analytics_read_role
    ...
    },
    {
      ...
      description: "Grant the new role to SYSADMIN. This is the single most important best practice — it ensures SYSADMIN can always manage objects owned by this role, preventing orphaned objects.",
      code: "GRANT ROLE analytics_read_role TO ROLE sysadmin;",
      tip: "If you forget this step and the role creates a table, only ACCOUNTADMIN will be able to manage that table — SYSADMIN cannot. This is a common exam scenario."
    },
    {
...
Every user has a **default role** — the role that becomes active when they establish a new Snowflake connection. Users can hold multiple roles and switch between them during a session.
...
<Diagram client:load
  ...
  description="The ACCOUNTADMIN role should be held by a minimum of two users (to avoid account lockout if one user is unavailable), never set as anyone's default role, always protected with multi-factor authentication, and used only for specific administrative tasks — never for day-to-day query work. Operational work should be performed with SYSADMIN, SECURITYADMIN, or a custom role."
  altText="Diagram showing ACCOUNTADMIN with four best-practice annotations: '2+ users assigned', 'Never set as DEFAULT_ROLE', 'Always use MFA', 'Use only for admin tasks — not daily queries'. A crossed-out icon shows a developer running SELECT queries as ACCOUNTADMIN, labelled as bad practice."
  imagePath="/images/diagrams/system-role-hierarchy.png"
/>

<CalloutBox client:load type="warning" title="ACCOUNTADMIN Best Practices — Exam Critical">
The COF-C02 exam frequently tests ACCOUNTADMIN best practices. Memorise all four:

1. **Assign ACCOUNTADMIN to at least 2 users** — if the sole ACCOUNTADMIN user is unavailable (forgotten password, employee departure), the account becomes unmanageable. Snowflake recommends a minimum of 2.
2. **Never set ACCOUNTADMIN as a user's DEFAULT_ROLE** — it should only be used when specifically needed for administrative tasks. Day-to-day connections should use a less-privileged default role.
3. **Enable MFA (Multi-Factor Authentication) for all ACCOUNTADMIN users** — ACCOUNTADMIN can access billing data, modify account parameters, and perform destructive operations. MFA is a critical security control.
4. **Restrict ACCOUNTADMIN to the most trusted personnel only** — typically limited to senior DBAs, security administrators, or a dedicated service account for automation.
</CalloutBox>
...
-- If any rows are returned, those users have ACCOUNTADMIN as their default — fix this!`}
/>
...
<CodeBlock client:load
  language="sql"
  title="Future Grants — Complete Reference"
...
## 9. SHOW GRANTS — Auditing Privileges
...
<CodeBlock client:load
  language="sql"
  title="SHOW GRANTS — All Variants"
...
A **managed access schema** is a special schema type where only the schema owner (or SECURITYADMIN/ACCOUNTADMIN) can grant privileges on objects within it — not the object owners themselves. This centralises privilege management and prevents privilege escalation.
...
<KeyTerms client:load
  ...
  terms={[
    ...
    {
      term: "PUBLIC",
      definition: "A system role automatically granted to every user in the account. Has no default privileges. Granting privileges to PUBLIC makes them available to all users — use with caution.",
      abbr: ""
    },
    {
      term: "Default role",
    ...
    {
      term: "Role hierarchy",
      definition: "The structure created when a role is granted to another role, causing the parent role to inherit all privileges of the child role. ACCOUNTADMIN → SYSADMIN → custom roles is the recommended pattern.",
      abbr: ""
    },
    {
      term: "LOGIN_NAME",
      definition: "The credential a user types at the Snowflake login screen. Can be set independently from the user's SQL identifier name — for example, set to an email address while keeping the SQL name shorter.",
      abbr: ""
    },
    {
...
<Flashcard client:load
  ...
  answer="1. ACCOUNTADMIN — highest privilege, manages billing, account settings, inherits SYSADMIN + SECURITYADMIN. 2. SYSADMIN — creates and manages databases, schemas, tables, warehouses. 3. SECURITYADMIN — manages users, roles, and grants (holds MANAGE GRANTS). 4. USERADMIN — creates users and roles only (no grant management). 5. PUBLIC — automatically granted to all users, no default privileges."
  category="System Roles"
/>

<Flashcard client:load
  question="What are the four ACCOUNTADMIN best practices that appear on the COF-C02 exam?"
  answer="1. Assign ACCOUNTADMIN to a minimum of 2 users to prevent lockout. 2. Never set ACCOUNTADMIN as anyone's DEFAULT_ROLE — use it only when needed for admin tasks. 3. Enable MFA (Multi-Factor Authentication) for all ACCOUNTADMIN users. 4. Restrict ACCOUNTADMIN to the most trusted staff only — do not use it for day-to-day queries."
...
<Flashcard client:load
  ...
  answer="GRANT ROLE child TO ROLE parent creates a role hierarchy — parent inherits all of child's privileges. GRANT ROLE my_role TO USER jane assigns a role directly to a user, giving them access to that role's privileges. Both are needed: custom roles must be granted to SYSADMIN (role-to-role) AND granted to users (role-to-user)."
...
<Quiz client:load
  ...
  options={[
    ...
    { label: "B", text: "The account becomes unmanageable for tasks requiring ACCOUNTADMIN — such as billing management or adding new users — because no one can access that role. The best practice is to always assign ACCOUNTADMIN to a minimum of 2 trusted users." },
  ...
  correct="B"
  explanation="Having only one ACCOUNTADMIN user is a critical operational risk. If that user is unavailable (forgotten password, employment termination, MFA device lost), the account cannot perform account-level tasks. Snowflake strongly recommends assigning ACCOUNTADMIN to at least 2 trusted users. SYSADMIN does not inherit ACCOUNTADMIN — it is the other way around (ACCOUNTADMIN inherits SYSADMIN). Snowflake Support cannot impersonate customers."
...
<Quiz client:load
  ...
  options={[
    { label: "A", text: "The table was created in the wrong schema — users need to check the full table path." },
...
<CheatSheet client:load
  title="User & Role Management — COF-C02 Quick Reference"
  sections={[
    {
      ...
      items: [
        ...
        { label: "PASSWORD", value: "Initial login password", note: "Hashed — never readable" },
    ...
    },
    {
      ...
      icon: "🔐",
      items: [
        { label: "ACCOUNTADMIN", value: "Everything — billing, account params, inherits all", note: "Min 2 users, never default" },
...
Snowflake's access control is built on a **role-based model** where privileges flow from objects to roles to users. The five system roles — **ACCOUNTADMIN, SYSADMIN, SECURITYADMIN, USERADMIN, and PUBLIC** — each have specific, non-overlapping responsibilities that the COF-C02 exam tests in detail.

For the exam, internalise these key principles:

- **ACCOUNTADMIN** is the top of the hierarchy — minimum 2 users, never a default role, always MFA-protected
- **SYSADMIN** creates objects — custom roles must roll up to SYSADMIN to prevent orphaned objects
- **SECURITYADMIN** manages access — it holds MANAGE GRANTS and inherits USERADMIN
- **USERADMIN** creates users and roles only — no grant management
- **Future grants** are essential but not retroactive — always pair with `ON ALL` for existing objects
