...
<KeyTerms client:load title="Key Terms — Network Security" terms={[
  ...
  {term: "Tri-Secret Secure", abbr: "TSS", definition: "Business Critical feature that encrypts Snowflake data using a composite key — one half held by Snowflake, the other held by the customer in their own KMS. Neither party alone can decrypt the data."},
...
<CalloutBox type="exam" title="Critical Rule — Block List Takes Precedence">
...
<CalloutBox type="exam" title="Private Link — Edition Requirement">
...
<CalloutBox type="warning" title="Tri-Secret Secure — Data Access Revocation">
Because the composite key requires the customer's key to function, the customer can **revoke Snowflake's ability to access their data** at any time simply by rotating or disabling their key in their KMS. This provides ultimate data sovereignty — even Snowflake cannot access your data without your key.
...
Even with strong AES-256 encryption, if an encryption key is somehow compromised, all data encrypted with that key is at risk indefinitely. Periodic rekeying limits the blast radius — data older than the rekeying period is already protected by a new key, so a compromised key only exposes data from a limited recent window.
</CalloutBox>
...
<CompareTable client:load title="MFA Token Caching: Enabled vs Disabled" leftLabel="Token Caching Enabled" rightLabel="Token Caching Disabled" rows={[
  ...
  {feature: "Security level", left: "Slightly lower — window of unchallenged access", right: "Higher — every connection is verified", winner: "right"},
  {feature: "User experience", left: "Better — fewer interruptions for data analysts", right: "Disruptive for frequent reconnects", winner: "left"},
...
<CalloutBox type="info" title="Security Integrations — Key Point for the Exam">
...
<CheatSheet client:load title="Network Security Quick Reference — COF-C02" sections={[
  {title: "Network Policy Basics", icon: "🛡️", items: [
  ...
  {title: "SHOW / DESCRIBE Commands", icon: "🔍", items: [
  ...
  {title: "Private Connectivity", icon: "🔒", items: [
  ...
  {title: "Authentication", icon: "🔑", items: [
  ...
  {title: "Edition Requirements", icon: "⭐", items: [
