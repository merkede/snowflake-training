...
<KeyTerms
  client:load
  title="Key Terms — Data Protection"
  terms={[
    ...
    { term: "Retention Period", abbr: "RP", definition: "The configurable number of days (0–90) that Time Travel data is preserved for a database, schema, or table." },
    ...
    { term: "AT / BEFORE", abbr: "—", definition: "SQL clauses used with SELECT, CREATE CLONE, and UNDROP to access historical data at a specific timestamp or offset." },
    { term: "UNDROP", abbr: "—", definition: "DDL command that restores a dropped table, schema, or database from Time Travel storage." },
  ]}
/>
...
Snowflake's **Time Travel** is one of its most powerful differentiating features. It allows you to access historical data — data that has been **updated**, **deleted**, or **dropped** — at any point within your defined retention period.
...
<Diagram
  ...
  description="Illustration of how data moves from Active Storage → Time Travel Storage → Fail-safe Storage → Purged, showing the retention timeline and what operations are available at each stage"
  altText="Timeline diagram showing Active, Time Travel (0-90 days), Fail-safe (7 days), and Purge stages"
  imagePath="/images/diagrams/time-travel-lifecycle.png"
/>
  ...
  sections={[
    {
      ...
      icon: "🕐",
      items: [
        { label: "AT", value: "TIMESTAMP => '2024-01-15 09:00:00'::TIMESTAMP_TZ", note: "Inclusive — data as of this exact time" },
        { label: "BEFORE", value: "TIMESTAMP => '2024-01-15 09:00:00'::TIMESTAMP_TZ", note: "Exclusive — data just before this time" },
      ]
    },
    {
      title: "Offset-Based",
...
<Flashcard
  ...
  answer="AT(TIMESTAMP => ...), AT(OFFSET => ...), and AT(STATEMENT => ...) — each also has a BEFORE variant. TIMESTAMP pins to a wall-clock time, OFFSET specifies seconds from now, and STATEMENT references a specific query ID."
/>
...
ALTER TABLE customers RENAME TO customers_new_empty;`,
      tip: "Always inspect the new table before renaming — it may contain data you need!",
    },
    {
      title: "Run UNDROP",
...
<Flashcard
  ...
  answer="Tables, Schemas, and Databases can all be restored with UNDROP — as long as they are within the Time Travel retention period and no newer object with the same name exists at the same level."
/>
...
<Diagram
  ...
  imagePath="/images/diagrams/zero-copy-cloning.png"
/>
...
<CompareTable
  ...
  rows={[
    ...
    { feature: "Duration", left: "0–90 days (configurable)", right: "7 days (fixed, non-configurable)", winner: "left" },
    { feature: "Who can access?", left: "You — via SQL (AT/BEFORE, UNDROP, CLONE)", right: "Snowflake Support only (no self-service)", winner: "left" },
    { feature: "Can you query data?", left: "Yes — full SQL access", right: "No — not directly accessible", winner: "left" },
    { feature: "Storage cost?", left: "Yes — billed per TB", right: "Yes — billed per TB", winner: "none" },
    { feature: "Applies to temp tables?", left: "Yes (max 1 day)", right: "No — excluded", winner: "none" },
    { feature: "Applies to transient tables?", left: "Yes (max 1 day)", right: "No — excluded", winner: "none" },
    { feature: "Configurable per object?", left: "Yes — table, schema, database level", right: "No", winner: "left" },
  ]}
/>

<CalloutBox type="warning" title="Fail-safe Is a Last Resort">
  Fail-safe is **not** a backup strategy. It exists only for catastrophic failure scenarios and can only be accessed by Snowflake staff. Do not rely on Fail-safe as your data recovery plan — use Time Travel and Zero-Copy Clones proactively.
</CalloutBox>
...
<Flashcard
  ...
  answer="Fail-safe is a fixed 7-day non-configurable period after the Time Travel period expires. Only Snowflake Support can access Fail-safe data — it is not self-service. It applies to permanent tables only (not transient or temporary)."
/>
...
<Diagram
  ...
  description="How DATA_RETENTION_TIME_IN_DAYS cascades from account → database → schema → table level, with more specific settings overriding broader ones"
  ...
  imagePath="/images/diagrams/time-travel-extensions.png"
/>
  ...
  sections={[
    {
      ...
      icon: "💰",
      items: [
        { label: "Active", value: "Current data in tables — always billed", note: "" },
        { label: "TT", value: "Historical data within retention period — billed per TB", note: "" },
        { label: "FS", value: "7-day Fail-safe storage beyond TT — billed per TB", note: "Permanent tables only" },
        { label: "Stage", value: "Internal stages — billed same as table storage", note: "" },
      ]
    },
    {
      ...
      icon: "📉",
      items: [
        { label: "Transient", value: "Use transient tables for staging data — no Fail-safe", note: "Saves 7 days of Fail-safe cost" },
...
<Quiz
  ...
  options={[
    { label: "A", text: "Yes — use UNDROP TABLE. The 5-day-old drop is within the 7-day Fail-safe period." },
    { label: "B", text: "No — the retention period was 3 days, so Time Travel has expired. Fail-safe is Snowflake-only." },
    { label: "C", text: "Yes — UNDROP always works within 90 days in Enterprise Edition." },
    { label: "D", text: "No — UNDROP only works for schemas and databases, not tables." },
  ]}
  correct="B"
  explanation="The table's Time Travel retention was set to 3 days, which has now expired (5 days ago). Even though you are on Enterprise Edition, the specific table's retention overrides the account/edition maximum. Fail-safe data cannot be accessed by users — only Snowflake Support can recover it, and only in extreme circumstances. The answer is B."
/>
...
<Quiz
  ...
  options={[
    { label: "A", text: "CREATE TABLE backup AS SELECT * FROM production — creates a full copy" },
    { label: "B", text: "CREATE TABLE backup CLONE production — zero-copy clone with metadata only" },
  ...
  correct="B"
  explanation="Zero-Copy Cloning (option B) is the correct answer. It creates an instant clone by copying only metadata — no physical data is duplicated. The clone initially costs nothing extra in storage and takes seconds, regardless of table size. Option A physically duplicates all 2 TB immediately. Option C involves significant data movement and latency. Option D only helps if you need to recover via UNDROP later, not for a backup-before-migration scenario."
/>
...
<Flashcard
  ...
  answer="No — transient and temporary tables have a maximum retention of 1 day, regardless of how you set the parameter. Setting it to 5 on a transient table will result in an error or be capped at 1."
/>
