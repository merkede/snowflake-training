...
description: "Master Snowflake zero-copy cloning — metadata-only copies, cloneable objects, Time Travel with clones, and privilege behaviour for the SnowPro Core exam"
moduleId: "cloning"
...
<KeyTerms client:load title="Key Terms — Zero-Copy Cloning" terms={[
  { term: "Zero-Copy Clone", abbr: "—", definition: "A metadata-only copy of a database, schema, or table that references the same underlying micro-partitions. No data is physically duplicated — instant and free at creation." },
  { term: "CREATE ... CLONE", abbr: "—", definition: "The SQL command to create a zero-copy clone of a database, schema, or table." },
  { term: "Clone + Time Travel", abbr: "—", definition: "Cloning AT or BEFORE a specific point in time, creating a snapshot of the object as it existed in the past." },
  { term: "Child Clone", abbr: "—", definition: "When cloning a database or schema, all child objects (tables, views, stages, etc.) are cloned recursively." },
  { term: "Micro-Partition Sharing", abbr: "—", definition: "Source and clone share the same physical micro-partitions until one side modifies data — then only changed partitions are written independently." },
]} />
...
Zero-copy cloning creates an **instant, metadata-only copy** of a database, schema, or table. No data is physically duplicated — the clone points to the same underlying micro-partitions as the source.

<Diagram client:load title="Zero-Copy Cloning Mechanics" description="Diagram showing: Source table points to micro-partitions MP1, MP2, MP3, MP4. After CREATE TABLE clone CLONE source, the clone table also points to the same MP1, MP2, MP3, MP4. No data copied — both share the same storage. When the clone later modifies data affecting MP2, a new MP2-prime is created for the clone while the source still points to the original MP2. Only the changed partition uses additional storage." altText="Source and clone sharing micro-partitions, with divergence only on modified partitions"   imagePath="/images/diagrams/zero-copy-cloning.png"
/>
...
-- Clone with Time Travel — snapshot from the past
...
<CompareTable client:load title="Cloneable vs Non-Cloneable Objects" leftLabel="Can Be Cloned" rightLabel="Cannot Be Cloned" rows={[
  ...
  { feature: "Schemas", left: "Yes — all child objects cloned recursively", right: "—", winner: "none" },
  { feature: "Databases", left: "Yes — all schemas and their children cloned", right: "—", winner: "none" },
  { feature: "Streams", left: "Yes (but offset is not preserved)", right: "—", winner: "none" },
  { feature: "Tasks", left: "Yes (cloned in suspended state)", right: "—", winner: "none" },
  { feature: "Stages", left: "Named internal stages (empty clone)", right: "External stages are referenced, not cloned", winner: "none" },
  { feature: "Pipes", left: "Yes (cloned in paused state)", right: "—", winner: "none" },
]} />

<CalloutBox type="warning" title="Important Clone Behaviours">
  - **Internal stages** are cloned but **empty** — files are not copied
  - **Streams** are cloned but the **offset resets** — historical change data is not preserved
  - **Tasks** are cloned in a **suspended state** — they do not automatically start running
...
<CalloutBox type="exam" title="Exam Focus: Clone AT vs UNDROP">
  Both CLONE AT and UNDROP can recover data. UNDROP restores a dropped object with its original name. CLONE AT creates a new object from a past snapshot — the original remains unchanged. CLONE AT works even if the object was not dropped (e.g., after an accidental UPDATE or TRUNCATE).
</CalloutBox>
...
<CalloutBox type="important" title="Privilege Behaviour on Clones">
  - **Table clones** do NOT inherit grants — the cloning role becomes the owner
...
<Diagram client:load title="Clone Storage Over Time" description="Timeline diagram: At creation (T0), clone uses 0 additional storage. At T1, source table receives an UPDATE affecting 10% of data — 10% new micro-partitions created for source. At T2, clone table receives a DELETE affecting 5% of data — 5% new micro-partitions created for clone. Total additional storage = only the changed partitions on each side. Shared (unchanged) partitions are counted once in storage billing." altText="Timeline showing zero initial cost then incremental storage as source and clone diverge"   imagePath="/images/diagrams/zero-copy-cloning.png"
/>
...
<StepByStep client:load title="Common Cloning Patterns" steps={[
  { title: "Development and testing", description: "Clone production to create a dev/test environment with real data — instantly and at zero initial cost.", code: `CREATE DATABASE prod_dev CLONE production;`, tip: "Clone regularly to keep dev environments fresh." },
  ...
  { title: "Data sandboxing", description: "Give analysts a clone to experiment with — modifications do not affect the source.", code: `CREATE SCHEMA analyst_sandbox CLONE analytics;\nGRANT USAGE ON SCHEMA analyst_sandbox TO ROLE analyst_role;` },
]} />
...
<CheatSheet client:load title="Zero-Copy Cloning Quick Reference" sections={[
  { title: "Key Facts", icon: "📋", items: [
    { label: "Storage at creation", value: "Zero — metadata only", note: "" },
...
<Quiz client:load category="Data Protection" question="How much additional storage does a zero-copy clone consume when first created?" options={[{label:"A",text:"50% of the source"},{label:"B",text:"100% of the source"},{label:"C",text:"Zero — it is metadata only"},{label:"D",text:"It depends on the table size"}]} correct="C" explanation="Zero-copy cloning creates a metadata-only pointer to the same micro-partitions. No data is physically duplicated, so storage cost at creation is zero." />
...
<Flashcard client:load category="Data Protection" question="What happens to grants when you clone a database?" answer="By default, grants are NOT inherited — the cloning role becomes owner of the clone and all children. Use COPY GRANTS to preserve the original grants. COPY GRANTS is available for database and schema clones but not individual table clones." />

<Flashcard client:load category="Data Protection" question="What is the difference between CLONE AT and UNDROP?" answer="UNDROP restores a dropped object with its original name. CLONE AT creates a new object from a past snapshot — works even if the source was not dropped (e.g., after accidental UPDATE/TRUNCATE). Both leverage Time Travel." />
