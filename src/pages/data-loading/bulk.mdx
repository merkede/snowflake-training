...
<Diagram
  ...
  description="End-to-end flow showing external data sources → stages → COPY INTO → Snowflake tables"
  altText="Data loading architecture showing cloud storage, stages, file formats, and target tables"
  imagePath="/images/diagrams/bulk-loading-architecture.svg"
/>
...
<Diagram
  ...
  imagePath="/images/diagrams/stage-types-comparison.svg"
/>
...
<Diagram
  ...
  description="Step-by-step visualisation of how COPY INTO processes files: validation → parallelisation → loading → error handling"
  altText="COPY INTO command execution flow diagram"
  imagePath="/images/diagrams/copy-into-execution-flow.svg"
/>
...
FROM (
  SELECT
    ...
    $2::INTEGER,                        -- Column 2 → customer_id
    TO_DATE($3, 'YYYY-MM-DD'),         -- Column 3 → order_date (converted)
    $4::DECIMAL(10,2),                 -- Column 4 → amount
    UPPER($5)                          -- Column 5 → status (uppercase)
...
<Diagram
  ...
  imagePath="/images/diagrams/load-history-monitoring.svg"
/>
...
✅ Use **stages** to organise files before loading
✅ Define **file formats** for reusability and consistency
✅ **COPY INTO** is the primary bulk loading command
✅ **Compress files** (100-250 MB each) for optimal performance
✅ Use **ON_ERROR** and **VALIDATION_MODE** to handle errors
✅ **PURGE = TRUE** to auto-delete loaded files
✅ Monitor loads with **COPY_HISTORY** information schema
✅ Use **dedicated warehouses** for large loads
