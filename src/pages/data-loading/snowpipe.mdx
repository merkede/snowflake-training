...
description: "Master Snowflake's Snowpipe serverless continuous data loading — event notifications, REST API, pipe management, and Snowpipe Streaming for the SnowPro Core exam"
moduleId: "snowpipe"
domain: "Data Loading"
---
...
<KeyTerms client:load title="Key Terms — Snowpipe" terms={[
  { term: "Snowpipe", abbr: "—", definition: "Serverless continuous data loading service that automatically ingests files as they arrive in a stage. No virtual warehouse required." },
  { term: "Auto-Ingest", abbr: "—", definition: "Snowpipe mode where cloud event notifications (SQS, Event Grid, Pub/Sub) automatically trigger file loading." },
  { term: "REST API", abbr: "—", definition: "Snowpipe's programmatic interface for manually triggering file ingestion via insertFiles() and checking status via insertReport()." },
  { term: "Pipe", abbr: "—", definition: "A Snowflake object (CREATE PIPE) that defines the COPY INTO statement used by Snowpipe." },
  { term: "Snowpipe Streaming", abbr: "—", definition: "Low-latency row-by-row ingestion via the Snowflake Ingest SDK — sub-second latency without staging files." },
]} />
...
Snowpipe is a **serverless continuous data ingestion** service. Unlike bulk COPY INTO which you run manually or on a schedule, Snowpipe automatically loads data as files arrive in a stage — with no virtual warehouse needed.

<Diagram client:load title="Snowpipe Architecture" description="Flow diagram: 1) Files arrive in cloud storage (S3/Blob/GCS). 2) Cloud event notification triggers (SQS for AWS, Event Grid for Azure, Pub/Sub for GCP). 3) Snowflake Cloud Services layer receives notification. 4) Serverless compute loads file into target table. 5) Load metadata updated. No virtual warehouse involved — all serverless." altText="Flow from file arrival through cloud notification to serverless loading into Snowflake table"   imagePath="/images/diagrams/snowpipe-architecture.png"
/>
...
<CheatSheet client:load title="Auto-Ingest by Cloud Provider" sections={[
  { title: "AWS", icon: "🟠", items: [
    { label: "Service", value: "S3 Event Notification → SQS Queue → Snowpipe", note: "" },
    { label: "Setup", value: "Configure S3 event on ObjectCreated → SQS → Snowflake notification integration", note: "" },
  ]},
  { title: "Azure", icon: "🔵", items: [
    { label: "Service", value: "Azure Event Grid → Snowpipe", note: "" },
    { label: "Setup", value: "Configure Blob Storage events → Event Grid → Snowflake notification integration", note: "" },
  ]},
  { title: "GCP", icon: "🟢", items: [
    { label: "Service", value: "Google Cloud Pub/Sub → Snowpipe", note: "" },
    { label: "Setup", value: "Configure GCS notifications → Pub/Sub topic → Snowflake notification integration", note: "" },
  ]},
]} />
...
<CompareTable client:load title="Snowpipe vs Bulk COPY INTO" leftLabel="Snowpipe" rightLabel="COPY INTO" rows={[
  ...
  { feature: "Compute", left: "Serverless — no warehouse needed", right: "Virtual warehouse required", winner: "left" },
...
<Diagram client:load title="Snowpipe Streaming Architecture" description="Unlike regular Snowpipe which requires files in a stage, Snowpipe Streaming uses the Snowflake Ingest SDK to write rows directly to Snowflake tables with sub-second latency. No intermediate files needed. Flow: Application → Snowflake Ingest SDK (Java/Python) → Snowflake table directly." altText="Direct row ingestion from application through SDK to Snowflake without staging files"   imagePath="/images/diagrams/snowpipe-architecture.png"
/>

<CalloutBox type="info" title="Snowpipe Streaming vs Regular Snowpipe">
  Regular Snowpipe: files → stage → load (minutes latency). Snowpipe Streaming: rows → SDK → table directly (sub-second latency). Streaming uses the Snowflake Ingest SDK for Java or Python. Popular for IoT, clickstream, and real-time event data.
</CalloutBox>
...
<CheatSheet client:load title="Snowpipe Quick Reference" sections={[
  { title: "Key Facts", icon: "🚰", items: [
    { label: "Compute", value: "Serverless — no warehouse needed", note: "" },
  ...
  { title: "Commands", icon: "⌨️", items: [
    ...
    { label: "Describe", value: "DESCRIBE PIPE p — shows notification_channel", note: "" },
  ]},
]} />
...
<Flashcard client:load category="Data Loading" question="How is Snowpipe billed compared to COPY INTO?" answer="Snowpipe: per-file serverless credits — no warehouse needed. COPY INTO: per-second warehouse credits based on warehouse size. Snowpipe is cost-efficient for frequent small files; COPY INTO is better for large batch loads." />
