...
description: "Master Snowflake data unloading — COPY INTO location, GET command, file formats, partitioning, and compression for the SnowPro Core exam"
moduleId: "unloading"
domain: "Data Loading"
---
...
<KeyTerms client:load title="Key Terms — Data Unloading" terms={[
  { term: "COPY INTO <location>", abbr: "—", definition: "SQL command to export data from a Snowflake table into files in a stage (internal or external). The reverse of COPY INTO <table>." },
  { term: "GET", abbr: "—", definition: "SQL command to download files from an internal stage to a local machine. Only works with internal stages, not external." },
  { term: "SINGLE", abbr: "—", definition: "COPY INTO option that writes all output to a single file instead of splitting across multiple files." },
  { term: "MAX_FILE_SIZE", abbr: "—", definition: "Controls the maximum size of each output file. Default is 16 MB for most formats." },
  { term: "PARTITION BY", abbr: "—", definition: "Organises unloaded files into a directory structure based on column expressions — useful for partitioned data lakes." },
  { term: "OVERWRITE", abbr: "—", definition: "When TRUE, overwrites existing files in the target location. Default is FALSE." },
]} />
...
Data unloading is the process of **exporting data from Snowflake tables into files** in a stage. This is the reverse of data loading — instead of COPY INTO a table, you COPY INTO a location.

<Diagram client:load title="Unloading Flow" description="Flow diagram: 1) Snowflake table contains the source data. 2) COPY INTO location command executes on a virtual warehouse. 3) Data is written as files (CSV, JSON, Parquet) to internal stage (@~, @%, @named) or external stage (S3/Blob/GCS). 4) Optionally, GET command downloads from internal stage to local filesystem." altText="Flow from Snowflake table through COPY INTO to stage files, then optionally GET to local"   imagePath="/images/diagrams/data-unloading-flow.svg"
/>

<CalloutBox type="exam" title="Exam Focus: Loading vs Unloading">
  COPY INTO **table** = loading (files to table). COPY INTO **location** = unloading (table to files). Both use a virtual warehouse. Snowpipe only supports loading — you cannot use Snowpipe to unload data.
</CalloutBox>
...
<CalloutBox type="info" title="Query-Based Unloading">
  You can unload from a full table or from a SELECT query (wrapped in parentheses). This lets you filter, transform, and join data before exporting — no need to create a temporary table first.
</CalloutBox>
...
PARTITION BY organises unloaded files into a directory structure based on column expressions — ideal for creating partitioned data lakes.
...
<CheatSheet client:load title="Compression by Format" sections={[
  ...
  { title: "JSON", icon: "📋", items: [
    { label: "Default", value: "GZIP", note: "" },
    { label: "Options", value: "GZIP, BZ2, BROTLI, ZSTD, DEFLATE, RAW_DEFLATE, NONE", note: "" },
  ]},
  { title: "Parquet", icon: "📊", items: [
...
<CheatSheet client:load title="Data Unloading Quick Reference" sections={[
  ...
  { title: "Key Options", icon: "⚙️", items: [
