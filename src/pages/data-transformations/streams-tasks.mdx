...
description: "Master Snowflake Streams for change data capture and Tasks for scheduled automation — building real-time data pipelines for the SnowPro Core exam"
---
...
<KeyTerms client:load title="Key Terms — Streams & Tasks" terms={[
  ...
  { term: "Insert-Only Stream", definition: "A stream type used exclusively on External Tables. Captures INSERT events only — representing newly loaded files." }
]} />
...
<CalloutBox type="exam" title="Exam Focus — Streams Do Not Store Data">
...
Stream: ORDERS_STREAM
    ...
    └── delta since T0:
          ...
          ROW 55   │ METADATA$ACTION = DELETE │ METADATA$ISUPDATE = FALSE
```
...
<Diagram client:load
  ...
  imagePath="/images/diagrams/streams-offset.png"
/>
...
<CompareTable client:load
  ...
  rows={[
    ...
    { feature: "Exam tip", left: "Default type — most questions", right: "Used for logs and event tables", winner: "none" }
  ]}
/>
...
<CodeBlock client:load language="sql" title="Creating Stream Types" code={`-- Standard stream (default) — tracks INSERT, UPDATE, DELETE
CREATE OR REPLACE STREAM orders_stream
  ON TABLE orders;

-- Append-only stream — tracks INSERT only (efficient for logs)
...
1. A DELETE row with the **old** column values — `METADATA$ISUPDATE = TRUE`
2. An INSERT row with the **new** column values — `METADATA$ISUPDATE = TRUE`
...
<Diagram client:load
  ...
  imagePath="/images/diagrams/streams-pipeline.png"
/>
...
- A stale stream must be **dropped and recreated** — you cannot "un-stale" it.
...
## 7. Tasks — Scheduled Automation Inside Snowflake
...
1. **Interval-based:** `SCHEDULE = '5 MINUTE'` — runs every N minutes (1 to 11,520 minutes / 8 days max).
2. **Cron-based:** `SCHEDULE = 'USING CRON 0 9 * * MON-FRI UTC'` — full cron expression. The timezone identifier is **mandatory** (e.g., `UTC`, `Europe/London`).

<CodeBlock client:load language="sql" title="CREATE TASK — Full Syntax Examples" code={`-- Basic task: virtual warehouse, interval schedule
...
- To enable the whole tree, resume child tasks first, then the root task — or use `SYSTEM$TASK_DEPENDENTS_ENABLE('root_task')`.
- A task tree can have **up to 1,000 tasks** in a single DAG.

<StepByStep client:load title="Building a Task DAG — Step by Step" steps={[
  {
  ...
  },
  {
    ...
    tip: "SYSTEM$TASK_DEPENDENTS_ENABLE resumes all child tasks automatically — you still need to resume the root task separately."
  },
  {
...
<Diagram client:load
  ...
  description="The root task runs every 15 minutes on a WAREHOUSE schedule. On success, child_task_1 runs immediately after. On success of child_task_1, child_task_2 runs. Each task uses the AFTER clause to express the dependency. The DAG terminates if any task fails — subsequent dependents do not run."
  ...
  imagePath="/images/diagrams/task-dag-structure.svg"
/>
...
<CompareTable client:load
  ...
  rows={[
    ...
    { feature: "Exam frequency", left: "High — know the keyword", right: "High — default approach", winner: "none" }
  ]}
/>

<CalloutBox type="important" title="Serverless Task Syntax — Exam Critical">
...
Note the `WHEN SYSTEM$STREAM_HAS_DATA(...)` clause — this is a **precondition**. The task only executes if the stream has unconsumed data, saving compute credits when there is nothing to process.

<CalloutBox type="note" title="WHEN Clause — Conditional Task Execution">
...
<CheatSheet client:load title="Streams & Tasks — Exam Cheat Sheet" sections={[
  {
    title: "Stream Types",
    icon: "🔄",
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Check staleness", value: "SHOW STREAMS → STALE column", note: "" }
    ]
  },
  {
    title: "Task Rules",
...
<Quiz client:load
  ...
  correct="B"
  explanation="All tasks are created in SUSPENDED state by default. The task will not execute until you run ALTER TASK mytask RESUME. This is one of the most common exam traps related to tasks — always remember to resume after creation."
/>
...
<Quiz client:load
  ...
  options={[
    { label: "A", value: "A task that runs without any SQL — purely using Snowpark Python" },
...
<Flashcard client:load
  category="Streams"
  question="What does a Snowflake Stream store — data or an offset?"
...
<Flashcard client:load
  ...
  answer="The stream becomes STALE. A stale stream cannot be used — it must be dropped and recreated. The SHOW STREAMS command exposes STALE (boolean) and STALE_AFTER (timestamp) columns to monitor staleness."
/>
