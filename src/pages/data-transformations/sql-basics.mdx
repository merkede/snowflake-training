...
description: "Master Snowflake-specific SQL features — VARIANT/semi-structured data, FLATTEN, window functions, QUALIFY, PIVOT/UNPIVOT, and unique SQL extensions for the SnowPro Core exam"
moduleId: "sql-basics"
...
<KeyTerms
  client:load
  title="Key Terms — Snowflake SQL Essentials"
  terms={[
    ...
    { term: "FLATTEN", abbr: "—", definition: "A Snowflake table function used with LATERAL to explode arrays or objects in a VARIANT column into individual rows." },
    ...
    { term: "QUALIFY", abbr: "—", definition: "A Snowflake-specific SQL clause that filters rows based on the result of a window function, avoiding the need for a subquery or CTE." },
    { term: "PIVOT", abbr: "—", definition: "A SQL operator that rotates rows into columns, turning distinct values in one column into separate column headers (wide format)." },
    { term: "UNPIVOT", abbr: "—", definition: "The reverse of PIVOT — rotates columns into rows, converting wide-format data into a tall/narrow format." },
    { term: "GENERATOR", abbr: "—", definition: "A Snowflake table function that produces synthetic rows without reading from any source table, useful for testing and data generation." },
    { term: "TABLESAMPLE", abbr: "—", definition: "A SQL clause that returns a random statistical sample of rows from a table, specified as a percentage." },
    { term: "PARSE_JSON", abbr: "—", definition: "A Snowflake function that converts a JSON-formatted string into a VARIANT value for semi-structured data processing." },
...
-- Basic path access — returns VARIANT
...
-- GET() function — equivalent to : operator
SELECT GET(payload, 'user') FROM events;

-- GET_PATH() function — dot-notation path
SELECT GET_PATH(payload, 'user.name')::STRING FROM events;

-- TYPEOF() — inspect the internal data type of a VARIANT value
SELECT
...
<Diagram
  client:load
  title="VARIANT Data Model — Path Access"
  ...
  imagePath="/images/diagrams/variant-path-access.svg"
/>
...
<CodeBlock
  client:load
  language="sql"
  title="FLATTEN — Exploding Arrays and Objects"
  code={`-- Table: users(id INT, profile VARIANT)
-- profile: {"name": "Bob", "tags": ["admin","editor","viewer"]}

-- Explode the tags array — one row per tag
SELECT
  u.id,
...
-- f.VALUE  — the element value (VARIANT)
-- f.KEY    — the key name (for objects, NULL for arrays)
-- f.INDEX  — the zero-based position in the array
-- f.PATH   — full path to the element from the root
-- f.SEQ    — sequence number unique per input row
-- f.THIS   — the containing array/object
...
-- OUTER => TRUE — keep rows even when the array is NULL or empty
SELECT u.id, f.value::STRING AS tag
FROM users u,
  LATERAL FLATTEN(INPUT => u.profile:tags, OUTER => TRUE) f;

-- RECURSIVE => TRUE — flatten nested arrays/objects recursively
...
<CalloutBox type="tip" title="OUTER => TRUE for Missing Arrays">
  If a row has a NULL or missing array path, a standard FLATTEN will silently drop that row from results. Use `OUTER => TRUE` to retain parent rows with no array elements — the FLATTEN output columns will be NULL for those rows.
</CalloutBox>
...
<Diagram
  ...
  imagePath="/images/diagrams/window-function-frame.png"
/>

<CodeBlock
  client:load
  language="sql"
  title="Window Function Examples — Ranking and Navigation"
...
-- LEAD() and LAG() — access rows ahead of or behind the current row
SELECT
  order_date,
  revenue,
  ...
  sections={[
    {
      ...
      icon: "🏆",
      items: [
        { label: "ROW_NUMBER()", value: "Unique sequential integer per partition — no ties", note: "Always unique, even for equal values" },
        ...
        { label: "CUME_DIST()", value: "Cumulative distribution — fraction of rows <= current", note: "Always > 0, always <= 1" },
      ]
    },
    {
      ...
      items: [
        ...
        { label: "FIRST_VALUE(col)", value: "First value in the window frame", note: "Frame spec matters — use UNBOUNDED" },
        { label: "LAST_VALUE(col)", value: "Last value in the window frame", note: "Defaults to CURRENT ROW — specify frame!" },
...
## QUALIFY — Snowflake's Filter on Window Functions
...
<CodeBlock
  client:load
  language="sql"
  title="QUALIFY — Deduplication and Top-N Filtering"
...
-- WITH QUALIFY — cleaner single-level query:
SELECT *
FROM orders
...
-- QUALIFY with a WHERE clause — WHERE runs before QUALIFY
SELECT *
FROM orders
...
<CompareTable
  client:load
  title="QUALIFY vs Subquery — Deduplication Approaches"
  leftLabel="QUALIFY (Snowflake)"
  rightLabel="Subquery / CTE"
  rows={[
    { feature: "Syntax complexity", left: "Single SELECT statement — no nesting", right: "Requires subquery or CTE wrapper", winner: "left" },
    { feature: "Standard SQL", left: "Snowflake extension — not portable", right: "Standard SQL — portable across databases", winner: "right" },
    { feature: "Readability", left: "Highly readable — intent is clear", right: "Can be verbose for simple deduplication", winner: "left" },
...
<Diagram
  ...
  imagePath="/images/diagrams/generator-vs-tablesample.svg"
/>
...
-- DECODE: value-based mapping (Oracle-style — Snowflake supports it)
...
<CalloutBox type="tip" title="NVL2 Argument Order — Easy to Mix Up">
  `NVL2(expr, not_null_result, null_result)` — the second argument is returned when `expr` IS NOT NULL, and the third argument when `expr` IS NULL. This is the opposite order from what many developers expect intuitively.
</CalloutBox>
...
<CodeBlock
  client:load
  language="sql"
  title="MATCH_RECOGNIZE — Pattern Detection"
...
<Quiz
  ...
  correct="B"
  explanation="In Snowflake, the colon (:) operator is used to navigate VARIANT paths. The double-colon (::) operator casts the result to a SQL type. So data:price::NUMBER is correct. Option A uses dot notation which is not valid at the top level of VARIANT access. Option C uses the PostgreSQL JSON arrow operator which Snowflake does not support. Option D (GET_PATH) would return a VARIANT, not a NUMBER — you would still need to cast it."
/>
...
<Flashcard
  ...
  answer="RANK() assigns the same rank to ties but then skips the next rank number — for example, two rows tied at rank 2 both get rank 2, and the next row gets rank 4 (not 3). DENSE_RANK() also assigns the same rank to ties but does NOT skip numbers — the next row after two tied rank-2 rows gets rank 3."
/>
...
<Flashcard
  ...
  answer="The evaluation order is: FROM → WHERE → GROUP BY → HAVING → WINDOW FUNCTIONS → QUALIFY → SELECT (column aliasing) → DISTINCT → ORDER BY → LIMIT/FETCH. QUALIFY runs after window functions are computed, which is why you can reference window function results directly in QUALIFY without a subquery."
/>
...
- [Data Loading — Staging and COPY INTO](/data-loading/bulk)
- [Performance Optimisation — Query Tuning](/performance/queries)
